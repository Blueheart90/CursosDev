<section>
    ...
    <hr class="mt-2 mb-6 ">
    @forelse ($course->audiences as $item)
        <article class="mt-4 card">
            <div class="bg-gray-100 card-body">
                @if ($audience->is($item))
                    <form wire:submit.prevent='update'>
                        <input wire:model='audience.name' class="w-full rounded-md " type="text">
                        @error('audience.name')
    ...
    @empty
        <p>No hay audiencias para mostrar</p>
    @endforelse
        ...
        <article class="card" x-show="open" x-cloak>
            <div class="bg-gray-100 card-body">
                <form wire:submit.prevent='store'>
                    <input wire:model='name' placeholder="Ingresa nombre de la audiencia" class="w-full rounded-md " type="text">
                    @error('name')
                        <span class="text-xs text-red-500 ">{{ $message }}</span>
                    @enderror
                    <div class="flex justify-end mt-2 ">
                        <button type="submit" class="px-4 py-2 ml-2 text-sm text-white bg-blue-500 rounded-md hover:bg-blue-600">Agregar Audiencia</button>
